...
_vol_inc=2;// [-2:0.1:5]
// Edge Depth
_edge_depth=2;// [1:0.25:10]
// Edge Border
_edge_width=1;// [0.1:0.1:3]
...
module case_right(main_pos,main_vol,edge_depth,edge_width,edge_tol,battery_length) {
 ...
 difference() {
  translate([-11.6,-17,-0.5]) // (8.1) fix bascule issue
   cylinder(h=3,r=1.9,center=true);
  translate([-14,-17,0])
   cube([5,5,6], center=true);
 }
 ...
 difference() { 
  translate([-8,7,SCREW_Z])
   ...
   translate([0.5,7,SCREW_Z])
     rotate([0,90,0])
      cylinder(h=15,r=2.1,center=true); // -> (8.1) make it larger since it was to tight
 }
...
// DEBUG insert only
//volume_right(_pos,_vol,_edge_depth,_edge_width,_edge_tol); 
//volume_left(_pos,_vol,_edge_depth,_edge_width);

// DEBUG cuts
//view_up(_pos,_vol,_edge_depth,_edge_width,_edge_tol,_batLg, 0, SCREW_Z); // focus: screw
//view_up(_pos,_vol,_edge_depth,_edge_width,_edge_tol,_batLg, 0, 10); // focus: ??
//view_up(_pos,_vol,_edge_depth,_edge_width,_edge_tol,_batLg, 0, 0); // focus: sensor


// Normal view 
if (_right || _left) view_both_cases(_pos,inc(_vol,_vol_inc),_edge_depth,_edge_width,_edge_tol,_batLg, _opening);
if (_print)
    view_print(_pos,inc(_vol,_vol_inc),_edge_depth,_edge_width,_edge_tol,_batLg, _opening);
